...
def main():
    ...
    parser.add_argument('--slp_weight', type=float, default=1.0, 
                        help='weight for sparse log prob loss')
    parser.add_argument('--dcl_weight', type=float, default=0.5, 
                        help='weight for depth consistency loss')
    parser.add_argument('--sfl_weight', type=float, default=100.0, 
                        help='weight for sparse flow loss')
    parser.add_argument('--dl_weight', type=float, default=20.0, 
                        help='weight for descriptor loss')
    ...
    parser.add_argument('--display_interval', type=int, default=50, 
                        help='iteration interval for image display')
    ...
    parser.add_argument('--training_patient_id', nargs='+', type=int, required=True, 
                        help='id of the training patients')
    ...
    parser.add_argument('--display_architecture', action='store_true', 
                        help='summarize the network architecture')
    parser.add_argument('--data_root', type=str, required=True, 
                        help='path to the training data')
    parser.add_argument('--log_root', type=str, required=True, 
                        help='root of the training logs')
    parser.add_argument('--precompute_root', type=str, required=True, 
                        help='root of the pre-compute data')
    ...
    parser.add_argument("--architecture_summary", action="store_true")
    parser.add_argument('--sequence_description', type=str, required=True,
                        help='description of training sequence')
    ...
    log_root = Path(args.log_root) / "depth_train_{}".format(args.sequence_description)
    ...
    descriptor_model = models.FCDenseNetFeature(
        ...
        growth_rate=8, out_chans_first_conv=16, feature_length=256)
    ...
    for cur_epoch in range(epoch, args.num_epoch + 1):
        ...
        if mean_loss < best_mean_loss:
            ...
            best_model_path = log_root / 'best_checkpoint_model.pt'
            utils.save_model(model=depth_estimation_model, optimizer=optimizer,
                             epoch=cur_epoch + 1, step=step, model_path=best_model_path,
                             validation_loss=mean_sf_loss)
