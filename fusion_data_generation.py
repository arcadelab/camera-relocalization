...
if __name__ == '__main__':
    ...
    parser.add_argument('--display_architecture', action='store_true', 
                        help='display the network architecture')
    ...
    for folder in folder_list:
        ...
        if (folder / "fusion_data.hdf5").exists():
            print("fusion_data hdf5 file already exists!")
            continue
        ...
        with torch.no_grad():
            for batch, (colors_1, sparse_depths_1, sparse_depth_masks_1, boundaries,
                        extrinsics_1, intrinsic_matrices, image_names, folders, start_h, start_w) in enumerate(fusion_loader):
                ...
                predicted_mean_depth_maps_1 = predicted_mean_depth_maps_1 * boundaries
                predicted_std_depth_maps_1 = predicted_std_depth_maps_1 * boundaries
                ...
                image_names = image_names.data.cpu().numpy()
            ...
            for idx in range(len(scale_list)):
                if state == "searching":
                    ...
                    if ratio < 0.3 or ratio > 3.0:
                        print("Frame: {}, Image: {}, abnormal scale: {}, ratio: {}".format(idx, image_name_list[idx], scale_list[idx], ratio))
                        continue
                    else:
                        recent_valid_index = idx
                        valid_index_list.append(idx)
            ...
            print("scale_list length: {}".format(len(scale_list)))
            print("valid_index_list length: {}".format(len(valid_index_list)))
            print("Write data to hdf5 file for further processing...")
                    
            tq = tqdm.tqdm(total=len(valid_index_list))
